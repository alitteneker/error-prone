...
java -cp loc/target/loc-1.0-SNAPSHOT.jar:loc/lib/javaparser-1.0.8.jar methodlevel.MethodPrinter $filename.tmp | sed 's/(),/,/' >> loc-metrics.csv.tmp

rm $filename.tmp

done

cat loc-metrics.csv.tmp | sed 's/java.tmp://' | sort | uniq > loc-metrics.csv
